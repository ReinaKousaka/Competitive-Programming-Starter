...
/**
 * AVL Tree, with random access (C++17)
 * reference and adapted from: https://github.com/jgreitemann/avl-tree
 * Declaration: AVL::tree<T> variable_name
 * Random Access: can access value with avl[i], where 0 <= i < avl.size()
 * APIs:
 * .insert(x), .clear(), .empty()
 * .remove(x): remove ALL nodes with value x
 * .remove_once(x): remove one node with value x
 * .find(x): return the iterator to the node with value x, return end() if not found
 * .find_greater(x): return the iterator to the smallest node > x, return end() if not found
 * .find_smaller(x): return the iterator to the largest node < x, return end() if not found
 * .count_greater(x): return the number of elements > x
 * .count_smaller(x): return the number of elements < x
 */
namespace AVL {
template <typename T, typename A = std::allocator<T> >	// C++17
class tree {
public:
...
public:
	...
	iterator insert(const T& t) {
		...
		do {
			if (parent->depth > branch_depth) break;
	...
	iterator find_greater(const_reference t) {
		node *ptr = root->left_child;
		node *res = nullptr;
		while (ptr) {
			if (ptr->data <= t) {
	...
	iterator find_smaller(const_reference t) {
		node *ptr = root->left_child;
		node *res = nullptr;
		while (ptr) {
			if (ptr->data >= t) {
	...
	int count_greater(const_reference t) {
		node *ptr = root->left_child;
		int res = 0;
		while (ptr) {
			if (ptr->data <= t) {
				...
				res++;
				if (ptr->right_child) res += ptr->right_child->n;
				ptr = ptr->left_child;
			}
		}
		return res;
	...
	int count_smaller(const_reference t) {
		node *ptr = root->left_child;
		int res = 0;
		while (ptr) {
			if (ptr->data >= t) {
				...
				res++;
				if (ptr->left_child) res += ptr->left_child->n;
				ptr = ptr->right_child;
			}
		}
		return res;
	...
	void remove_once(const_reference t) {
		iterator it = find(t);
		if (it != end()) erase(it);
	}
...
int main() {
	...
	while (n--) {
		...
		if (op == 1) {
			...
			avl.remove_once(x);		// only remove 1 element
		} else if (op == 3) {
			res = avl.count_smaller(x) + 1;
		} else if (op == 4) {
			res = avl[x - 1];
		} else if (op == 5) {
			auto it = avl.find_smaller(x);
			assert(it != avl.end());
			res = *it;
		} else {
			auto it = avl.find_greater(x);
			assert(it != avl.end());
			res = *it;
		}
		if (op > 2) {
